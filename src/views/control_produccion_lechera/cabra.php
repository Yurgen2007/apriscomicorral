...
$condicionActualNormalizada = strtolower(trim(str_replace('í', 'i', $condicionActual['condicion_especial'] ?? '')));
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        .goat-production-page { max-width: 1180px; }
        .goat-hero, .goat-card {
            background: #fff;
        ...
        .goat-hero h2 { margin: 0 0 5px; color: #402b16; }
        .goat-hero p { margin: 0; color: #806b58; }
        .goat-grid {
            display: grid;
            grid-template-columns: minmax(0, 1.5fr) minmax(280px, 1fr);
            gap: 22px;
        }
        .goat-card { padding: 22px; margin-bottom: 22px; }
        .goat-card h3 { margin: 0 0 16px; color: #402b16; }
        .goat-table-wrap { overflow-x: auto; border: 1px solid #ead7bf; border-radius: 10px; }
        .goat-table { width: 100%; min-width: 620px; border-collapse: collapse; }
        .goat-table th { padding: 12px; background: #ead0aa; color: #3a2614; text-align: left; white-space: nowrap; }
        .goat-table td { padding: 12px; border-top: 1px solid #f0e2d1; color: #4b3522; white-space: nowrap; }
        .lactation-item { padding: 14px; margin-bottom: 10px; background: #fffaf4; border: 1px solid #ead7bf; border-radius: 12px; }
        .lactation-item strong { color: #402b16; }
        .badge { display: inline-block; padding: 3px 8px; border-radius: 20px; background: #ead0aa; color: #5c3e20; font-size: .78rem; }
        .goat-year-filter {
            display: flex;
            align-items: end;
            gap: 10px;
        ...
        @media (max-width: 500px) {
            .goat-hero { align-items: flex-start; flex-wrap: wrap; }
            .goat-hero img { width: 70px; height: 70px; }
            .goat-year-filter { width: 100%; margin-left: 0; }
            .goat-year-filter select { width: 100%; }
        }
    </style>
</head>
<body>
        ...
        <section class="goat-card">
            ...
            <div class="goat-table-wrap">
                <table class="goat-table">
                    <thead><tr><th>Fecha</th><th>Lactancia</th><th>Condición</th><th>Turno</th><th>Litros</th><th>Acciones</th></tr></thead>
                    <tbody>
                    <?php foreach ($controles as $control): ?>
                        <tr>
                            <td><?= e($control['fecha_registro']) ?></td>
                            <td><?= $control['id_lactancia'] ? 'Lactancia #' . (int)($control['numero_lactancia'] ?? $control['id_lactancia']) : 'Sin vincular' ?></td>
                    ...
                    <?php endforeach; ?>
                    <?php if (empty($controles)): ?><tr><td colspan="6">No hay registros para esta cabra.</td></tr><?php endif; ?>
                    </tbody>
                </table>
            </div>
        </section>
            ...
            <?php if (empty($lactancias) && !empty($controles)): ?>
                ...
                <?php if ($registrosSinLactancia): ?>
                    <div class="lactation-item">
                        ...
                        <?php if ($condicionActualNormalizada === 'vacia'): ?>
                            <br>Fecha de cierre sanitario: <?= e(substr($condicionActual['fecha_control'], 0, 10)) ?>
                        <?php endif; ?>
                        <br>Duración registrada: <?= $intervaloSinLactancia ? (int)(($intervaloSinLactancia->y * 12) + $intervaloSinLactancia->m) . ' meses (' . (int)$intervaloSinLactancia->days . ' días)' : '0 días' ?>
